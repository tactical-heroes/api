...
Для написанных вручную файлов в `src`, `tests` и `tools` проверяются два правила:

- `ClosingBraces_Should_NotFollowBlankLines_When_SourceCodeIsDeclared` — перед
  закрывающей фигурной скобкой не должно быть пустой строки.
- `MethodDeclarations_Should_BeSeparatedByBlankLine_When_MembersAreAdjacent` —
  методы и конструкторы отделяются от соседних членов типа пустой строкой,
  расположенной перед атрибутами и комментариями следующего члена. Для групп
  полей и свойств дополнительная пустая строка не требуется.

Сгенерированные файлы и миграции исключены. Проверки используют синтаксическое
дерево Roslyn, поэтому скобки внутри строк и комментариев не считаются кодом.

В `.editorconfig` также включено правило `IDE2002`: `dotnet format` удаляет
пустые строки между последовательными закрывающими скобками. Остальные случаи
покрывают тесты выше, поскольку встроенный форматтер не требует пустую строку
между методами. StyleCop `SA1516` дополнительно требует разделять свойства,
поэтому его не подключаем для принятого в проекте стиля.

Проверка форматирования, выполняемая также в CI:

```powershell
dotnet format --verify-no-changes --severity warn --no-restore
```

Для автоматического исправления поддерживаемых форматтером правил уберите
`--verify-no-changes`; диагностика архитектурных тестов указывает файл и строку
для остальных исправлений.
